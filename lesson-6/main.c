 ...
 */
void putFoodSeed(struct food *fp, int color)
{
    int max_x = 0, max_y = 0;
    char spoint[2] = {0};
 ...
 */
void putFood(struct food f[], size_t number_seeds, int color)
{
    for (size_t i = 0; i < number_seeds; i++)
    {
        putFoodSeed(&f[i], color);
    }
}

void refreshFood(struct food f[], int nfood, int color)
{
...
_Bool haveEat(struct snake_t *head, struct food f[])
{
    ...
    {
        if (f[i].put_time && f[i].enable && head->x == f[i].x && head->y == f[i].y)
        {
            return 1;
        }
    }
...
{
    struct tail_t *last = &head->tail[head->tsize - 1];
    switch (head->direction)
    {
    case LEFT:
        head->tail[head->tsize] = (tail_t){last->x + 1, last->y};
        break;
    case RIGHT:
        head->tail[head->tsize] = (tail_t){last->x - 1, last->y};
        break;
    case UP:
        head->tail[head->tsize] = (tail_t){last->x, last->y + 1};
        break;
    case DOWN:
        head->tail[head->tsize] = (tail_t){last->x, last->y - 1};
        break;
...
void update(struct snake_t *head, struct food f[], const int32_t key, const int food_color)
{
    clock_t begin = clock();
    go(head);
    goTail(head);
    ...
    refreshFood(food, SEED_NUMBER, food_color); // Обновляем еду
...
void initGame()
{
    ...
    putFood(food, SEED_NUMBER, FOOD_COLOR); // Кладем зерна
}
...
int main()
{
    ...
    while (key_pressed != STOP_GAME)
    {
        ...
        for (int i = 0; i < PLAYERS; i++)
        {
            update(snakes[i], food, key_pressed, FOOD_COLOR);
            if (isCrush(snakes[i]))
            {
                break; //!!!!!!
            }
